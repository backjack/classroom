...
class MethodController extends AbstractController {
    ...
    public function bankAction(){
        ...
        $title = __('Payment for Enrollment');
        $student = $this->getStudent();
        $this->data['bank_instructions'] = '<h2>'.__('Your Invoice id').': '.$id.'</h2><h2>'.__('Amount').': '.price($this->invoice()->amount).'</h2>'.$this->data['bank_instructions'];
        $this->sendEmail($student->email,$title,$this->data['bank_instructions'],$this->getServiceLocator());
    ...
    public function paypalAction()
    {
       ...
       // return $this->redirect()->toRoute('application/default',['controller'=>'callback','action'=>'paypal']);
        ...
        $transaction = $gateway->authorize(array(
            ...
            'description'   => __('Enrollment for').' '.getCart()->getTotalItems().' '.__('items'),
    ...
    public function twocheckoutAction()
    {
        ...
        $transaction = $gateway->purchase(array(
            ...
            'cart'=>        [
                    [
                    'type'     => 'product',
                    'name'     => __('Enrollment for').' '.getCart()->getTotalItems().' '.__('items'),
                    'quantity' => 1,
    ...
    public function payusendAction()
    {
        $id = getCart()->getInvoice();
        $this->loadValues('payu',$id,true);
        ...
        try{
            ...
            if(isset($setTransactionResponse['payu_easyplus_url'])) {
                ...
                $this->flashMessenger()->addMessage($json['error']);
                return $this->goBack();
