...
class CourseController extends AbstractController {
    ...
    public function introAction(){
        ...
        if($firstClass){
            $classLink =  $this->url()->fromRoute('view-class',['classId'=>$firstClass->lesson_id,'sessionId'=>$firstClass->session_id]);
        ...
        $output['forumTopics'] = $this->bladeHtml('application.forum.topics',$bladeData);
        return $output;
    }
    ...
    public function classAction(){
        ...
        if($previousClass){
            $previous = $this->url()->fromRoute('view-class',['classId'=>$previousClass->lesson_id,'sessionId'=>$sessionId]);
        ...
        if($lectureTable->getTotalLectures($classId)>0){
            ...
            $next = $this->url()->fromRoute('view-lecture',['lectureId'=>$nextRow->lecture_id,'sessionId'=>$sessionId]);
        ...
        $output['customCrumbs'] = [
            ...
            $this->url()->fromRoute('application/default', ['controller' => 'course', 'action' => 'intro','id'=>$sessionId])=>__('Introduction'),
            '#'=>__('Class Details')
        ];
        return $output;
    }
    ...
    public function lectureAction(){
        ...
        $form->createSelect('account_id[]',__('Recipients (Admins/Instructors)'),$options,true);
        ...
        $output['customCrumbs'] = [
            ...
            $this->url()->fromRoute('course-details',['id'=>$sessionId,'slug'=>safeUrl($this->getSession($sessionId)->session_name)])=>__('Course Details'),
            $this->url()->fromRoute('application/default', ['controller' => 'course', 'action' => 'intro','id'=>$sessionId])=>__('Introduction'),
            $this->url()->fromRoute('view-class',['classId'=>$lecture->lesson_id,'sessionId'=>$sessionId])=>__('Class Details'),
            '#'=>__('Lecture')
        ];
    ...
    public function bookmarkAction(){
        ...
        if($this->request->isPost()){
            ...
            if($bookMarkTable->addBookMark($this->getId(),$lecturePageId,$sessionId)){
                ...
                $status = false;
                $message = __('bookmark-already-exists');
            }
    ...
    private function verifyClass($id,$session){
            ...
            if(!empty($total)){
                ...
                foreach($assignments as $assignment){
                    ...
                    if(!$assignmentSubmissionTable->hasSubmission($this->getId(),$assignment->assignment_id)){
                        ...
                        $subject = __('You have Homework for').' '.$previousClass->lesson_name;

                        $message = __('homework-notification-mail',['homework'=>$assignment->title,'class'=>$previousClass->lesson_name,'link'=>$this->absoluteRoute('application/submit-assignment',['id'=>$assignment->assignment_id])]);
                ...
                if($firstAssignment){
                    ...
                    $this->flashMessenger()->addMessage(__('homework-reminder-flash',['title'=>$firstAssignment->title,'class'=>$previousClass->lesson_name]));
      ...
      /*      $this->flashMessenger()->addMessage('The class "'.$lessonRow->lesson_name.'" is scheduled to start on '.date('d/M/Y',$lessonRow->lesson_date));
            ...
            $this->flashMessenger()->addMessage(__('class-starts-on',['class'=>$lesson->lesson_name,'date'=>date('d/M/Y',$lessonRow->lesson_date)]));
            // $this->redirect()->toRoute('course-details',['id'=>$session]);
            $this->redirect()->toRoute('application/default',['controller'=>'course','action'=>'intro','id'=>$session]);
    ...
    public function loglectureAction(){
        ...
        if($this->request->isPost())
        {
            ...
            if($next){
                $this->redirect()->toRoute('view-lecture',['lectureId'=>$next->lecture_id,'sessionId'=>$session]);
            }
            else{
                ...
                foreach($allLectures as $row){
                    ...
                    if(!$sessionLogTable->hasAttendance($this->getId(),$session,$row->lecture_id)){
                        $this->flashMessenger()->addMessage(__('outstanding-lectures'));
                        $this->redirect()->toRoute('view-lecture',['lectureId'=>$row->lecture_id,'sessionId'=>$session]);
                    }
                ...
                if(!empty($lessonRow->test_required) && !empty($lessonRow->test_id) && $testTable->recordExists($lessonRow->test_id) && !$studentTestTable->passedTest($this->getId(),$lessonRow->test_id)){
                   ...
                   // $this->flashMessenger()->addMessage('You are required to take this test in order to complete the \''.$lessonRow->lesson_name.'\' class');
                    ...
                    if($nextClass){
                        ...
                        $this->redirect()->toRoute('view-class',['sessionId'=>$session,'classId'=>$nextClass->lesson_id]);
                    }
                    else{
                        //classes are over
