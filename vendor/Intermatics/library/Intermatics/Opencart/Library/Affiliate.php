	...
	public function __construct($registry) {
		...
		if (isset($this->session->data['affiliate_id'])) { 
			...
			if ($affiliate_query->num_rows) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET ip = '" . $this->db->escape($this->oc_request->server['REMOTE_ADDR']) . "' WHERE affiliate_id = '" . (int)$this->session->data['affiliate_id'] . "'");
			} else {
				$this->logout();
			}
		}
